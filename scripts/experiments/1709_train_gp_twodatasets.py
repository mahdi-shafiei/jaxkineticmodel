...
def main():
    ...
    glycolyse_GP1, time_points_GP1, y0_GP1, dataset_GP1 = load_model_glucose_pulse_FF_datasets(
        f"{dataset1_name}.csv", dilution_rate="0.1", y0_dict=y0_dict
    )
    glycolyse_GP2, time_points_GP2, y0_GP2, dataset_GP2 = load_model_glucose_pulse_FF_datasets(
        f"{dataset2_name}.csv", dilution_rate="0.1", y0_dict=y0_dict
    )
    ...
    parameters_step_list = pd.DataFrame(parameters_step_list).to_csv(
        f"results/EXP4_Glycolysis_Fitting_Datasets/{date_today}_parameters_step50_{dataset1_name}_{dataset2_name}_{n_iter}.csv"
    )
    final_params = pd.DataFrame(pd.Series(params_init))
    final_params.to_csv(
        f"results/EXP4_Glycolysis_Fitting_Datasets/{date_today}_optimized_parameters_{dataset1_name}_{dataset2_name}_{n_iter}.csv"
    )
