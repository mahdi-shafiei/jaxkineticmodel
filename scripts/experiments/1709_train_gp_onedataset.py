...
def main():
    ...
    glycolyse_GP1,time_points_GP1,y0_GP1,dataset_GP1=load_model_glucose_pulse_FF_datasets(f"{dataset_name}.csv",dilution_rate="0.1",y0_dict=y0_dict)
...
#(grads1[key]+grads2[key]+grads3[key])/3
    ...
    parameters_step_list=pd.DataFrame(parameters_step_list).to_csv(f"results/EXP4_Glycolysis_Fitting_Datasets/{date_today}_parameters_step20_{dataset_name}_{n_iter}.csv")
    final_params=pd.DataFrame(pd.Series(params_init))
    final_params.to_csv(f"results/EXP4_Glycolysis_Fitting_Datasets/{date_today}_optimized_parameters_{dataset_name}_{n_iter}.csv")
