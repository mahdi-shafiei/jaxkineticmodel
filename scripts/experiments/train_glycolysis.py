...
loss_targets_glucose_pulse = [0, 1, 2, 4, 5, 6, 10, 12, 14, 20, 21, 22]
...
for step in range(200):
    ...
    for key in grads1.keys():
        grads[key] = (
            grads1[key] + grads2[key] + grads3[key] + grads4[key]
        ) / 4  # +grads5[key]+grads6[key]+grads7[key]+grads8[key]+grads9[key])/9
...
losses_per_iterations_added.to_csv(
    "results/EXP4_Glycolysis_Fitting_Datasets/1309_losses_for_9_datasets_literature_parameter_init.csv"
)
params_to_save = pd.DataFrame(pd.Series(params_init))
params_to_save.to_csv(
    "results/EXP4_Glycolysis_Fitting_Datasets/1309_params_gradient_descent_federated_literature_parameter_init.csv"
)
