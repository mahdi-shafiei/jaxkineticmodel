...
#if sbml model
name="Kinetic glycolysis assay model.sbml"
...
time_points=np.linspace(0,150,20)
...
for i in range(len(initial_values)):
    plt.scatter(tensor_timepoints.detach().numpy(),predicted_c.detach().numpy()[:,i],label=list(initial_concentration_dict.keys())[i])
# plt.yscale("log")
plt.legend()
plt.show()
...
df.to_csv("data/rawdata_"+name+"_p"+str(len(parameters))+".csv")