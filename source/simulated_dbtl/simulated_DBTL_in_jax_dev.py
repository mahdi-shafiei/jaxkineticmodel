...
class DesignBuildTestLearnCycle:
    ...
    def BUILD_simulate_perturbed_strains(self, strains_perturbed, plot=False):
    ...
    def TEST_add_noise(self,values,percentage,type="homoschedastic"):
        ...
        if type=="homoschedastic":            
            ...
            for targ in self.target:
                ...
                noised_values[targ]=noised_values
        ...
        if type=="heteroschedastic":
            ...
            for targ in self.target:
                ...
                values_new=np.random.normal(values,percentage*values[targ])
                values_new[values_new<0]=0
                noised_values[targ]=noised_values
...
strain_designs=dbtl_cycle.DESIGN_generate_strains(elements=9,samples=30,replacement=True)
values=dbtl_cycle.BUILD_simulate_perturbed_strains(strain_designs,plot=False)

noised_values=dbtl_cycle.TEST_add_noise(values=values,percentage=0.05)


data=dbtl_cycle.TEST_format_dataset(strain_designs=strain_designs,
                               production_values=values,
